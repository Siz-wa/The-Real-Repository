...
class CityDataModel{
    ...
    public function cityDataModelHandler($cityName){

        $genderData = $this->getCityDataGender($cityName);
        $ageGroupData = $this->getCityAgeGroup($cityName);
        $monthlyCitySpendingpData = $this->getMonthlySpendingByCity($cityName);
        $cityUsersData = $this->getCityUsers($cityName);
    ...
    public function getCityUsers($cityName){
        try{
            $query ="
            SELECT * 
            FROM customer
    ...
    public function getCityAgeGroup($cityName){
        try{
            $query ="
            SELECT 
            CASE 
            ...
            END AS ageGroup,
            COUNT(*) AS groupCount,
            ROUND((COUNT(*) / (SELECT COUNT(*) FROM customer WHERE city = :city)) * 100, 2) AS ageGroupPercentage
            FROM 
                customer
            WHERE
                city = :city
            ...
            $ageGroup->execute([':city'=> $cityName]);
    ...
    public function getMonthlySpendingByCity($cityName){
        try{
            $query = "
            SELECT 
            ...
            WHERE 
                ...
                AND (c.city = :cityName OR (:cityName = 'Others' AND (c.city IS NULL OR c.city = '')))
            GROUP BY 
                MONTH(p.created_at)
            ORDER BY 
                MONTH(p.created_at)
