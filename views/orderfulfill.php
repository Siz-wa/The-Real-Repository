                                ...
                                <div class="flex h-full flex-col pb-16">
                                    ...
                                    <div class="perfect-scrollbar relative -mr-3.5 h-full grow pr-3.5">
                                        <div class="space-y-1">
                                            ...
                                            <button class="btn btn-primary w-full" type="button"  @click="submitMultipleDeliveries" x-show="selectedTasks.length > 0">
...
<!-- 
                                                                ...
                                                                <div class="flex w-max items-center justify-between">
                                                                    <div class="flex-shrink-0 ltr:mr-2.5 rtl:ml-2.5">
                                                                        <template x-if="task.path">
                                                                            <img :src="task.path" class="h-8 w-8 rounded-full object-cover" alt="avatar">
                                                                        </template>
                                ...
                                <div class="flex min-h-screen items-center justify-center px-4" @click.self="viewTaskModal = false">
                                    <div x-show="viewTaskModal" x-transition="" x-transition.duration.300="" class="panel my-8 w-[90%] max-w-lg overflow-hidden rounded-lg border-0 p-0 md:w-full">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 bg-[#fbfbfb] py-3 text-lg font-medium ltr:pl-5 ltr:pr-[50px] rtl:pr-5 rtl:pl-[50px] dark:bg-[#121c2c]">
                                            ...
                                            <p>OID#</p><div x-text="selectedTask.id"></div>
                                        ...
                                        <div class="p-5">
                                            <img :src="selectedTask.product"  style="width: 300px; height: 300px;" alt="product">
                                            <div class="prose text-white-dark mb-5" x-html="selectedTask.description"></div>
                                           
                                            <div class="mb-5">
                                                <p class="whitespace-nowrap text-base font-semibold group-hover:text-primary">Customer Full Name: </p><span  x-html="selectedTask.assignee"></span>
                                                       
                                            </div>
                                            <div class="mb-5"><p class="whitespace-nowrap text-base font-semibold group-hover:text-primary">Customer Contact No#: </p ><span  x-html="selectedTask.phone"></span>      
</div>
                                            <div class="mb-5"><p class="whitespace-nowrap text-base font-semibold group-hover:text-primary">Address:</p> <span x-html="selectedTask.address"></span>  </div>
        ...
        <script>
            ...
            const delivery = <?= json_encode($delivery)?>;
            const defaultParams = {
                id: null,
                title: '',
                description: '',
            ...
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.data('todolist', () => ({
                    ...
                    allTasks:delivery.map((deliver)=>({
                        id: deliver.orderID,
                        title: deliver.productName+' OID#'+deliver.orderID+' Ordered by '+deliver.fname+' '+deliver.lname,
                        descriptionText: deliver.description,
                        description: deliver.description,
                        path:  `data:image/jpeg;base64,${deliver.pfPicture}`,
                        tag: deliver.status,
                        assignee: deliver.fname+' '+deliver.lname,
                        product: `data:image/jpeg;base64,${deliver.image}`,
                        date: 'Expected Arrival: '+deliver.requiredDate,
                ...
                toggleTaskSelection(task) {
                    ...
                    if (index > -1) {
                        this.selectedTasks.splice(index, 1); // Unselect
                    } else {
                        this.selectedTasks.push(task); // Select
                    }
                },
                ...
                submitMultipleDeliveries() {
                    const payload = this.selectedTasks.map(task => ({
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            this.showMessage('Deliveries created successfully');
                            this.selectedTasks = []; // Clear selection
                        } else {
                            this.showMessage('Some inserts failed', 'error');
                        }
                    });
                },
                    ...
                    searchTasks(isResetPage = true) {
                       ...
                       const keyword = this.searchTask.trim().toLowerCase();
                        this.filteredTasks = res.filter((d) => d.title?.toLowerCase().includes(keyword));
                        this.getPager();
                    },
                    getPager() {
                        setTimeout(() => {
